...
- Produces:
  - `parse_position_names(scene: MvrScene) -> dict[str, str]` — UUID→Name aus `AUXData/Position` (direkte Kinder per MVR-Spec; Attribute `uuid`, `name`); leeres Dict wenn kein AUXData.
...
- Akkordeon: Sektion `transition: flex-grow 520ms cubic-bezier(0.16,1,0.3,1)`; aktiv S1 `flex:0 1 auto` (wächst nicht; inaktiv `0 0 auto`), S2/S3 aktiv `flex:1 1 auto`; Wrapper `max-height` 0↔900px gleiche Kurve; Zoom `scale(0.96) translateY(-14px)`→`scale(1)`, `transform 520ms cubic-bezier(0.16,1,0.3,1), opacity 360ms ease`, origin top left. Titel-`font-size`-Transition 420ms; S1 aktiv 40px / S2 S3 32px / kollabiert 21px; ls 0.02em; S1 line-height 1.05. Eyebrows Mono 11px ls 0.14em `--blue-300`.
